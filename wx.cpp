...
#define	WX_STAYUP	20000		// time for wx display to stay up, millis() -- nice if == GPATH_LINGER
 ...
 */
static bool getCurrentWX (const LatLong &ll, bool is_de, WXInfo *wip, char ynot[])
{
 ...
 * this is used by updateWiFi() for persistent display, use showDEWX() below for transient display
 */
 ...
 * this is used by updateWiFi() for persistent display, use showDXWX() below for transient display
 */
 ...
 * if assigned to a box just update, else show in plot1_b then arrange to revert after WX_STAYUP
 */
void showDXWX()
{
    if (plot3_ch == PLOT3_DXWX) {
        (void) updateDXWX (plot3_b);
    } else {
        (void) updateDXWX (plot1_b);
