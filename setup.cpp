...
static StringPrompt string_pr[N_SPR] = {
    ...
    {0, {280, R2Y(1), 70, PR_H}, {350, R2Y(1), 270, PR_H}, "host:", gpsdhost, NV_GPSDHOST_LEN, 0},
    {0, {110, R2Y(2), 65, PR_H}, {180, R2Y(2), 480, PR_H}, "SSID:", ssid, NV_WIFI_SSID_LEN, 0},
    {0, {670, R2Y(2),110, PR_H}, { 10, R2Y(3), 789, PR_H}, "Password:", pass, NV_WIFI_PW_LEN, 0},

    // page 2

    {1, {110, R2Y(0), 60, PR_H}, {170, R2Y(0), 330, PR_H}, "host:", dxhost, NV_DXHOST_LEN, 0},
    {1, {510, R2Y(0), 60, PR_H}, {580, R2Y(0),  80, PR_H}, "port:", NULL, 0, 0},               // shadowed

    {1, {110, R2Y(1), 60, PR_H}, {170, R2Y(1), 330, PR_H}, "host:", ntphost, NV_NTPHOST_LEN, 0},

    {1, {270, R2Y(2), 80, PR_H}, {370, R2Y(2), 110, PR_H}, "dTemp:", NULL, 0, 0},              // shadowed
    {1, {530, R2Y(2), 80, PR_H}, {630, R2Y(2), 110, PR_H}, "brMin:", NULL, 0, 0},              // shadowed

    {1, { 10, R2Y(3), 80, PR_H}, {110, R2Y(3), 110, PR_H}, "CntDn:", NULL, 0, 0},              // shadowed
    {1, {270, R2Y(3), 80, PR_H}, {370, R2Y(3), 110, PR_H}, "dPres:", NULL, 0, 0},              // shadowed
    {1, {530, R2Y(3), 80, PR_H}, {630, R2Y(3), 110, PR_H}, "brMax:", NULL, 0, 0},              // shadowed
 ...
 * N.B. cluster map and kx3 use two bools to create 3 states
 */
static BoolPrompt bool_pr[N_BPR] = {
    ...
    {0, {210, R2Y(1),  60, PR_H}, {280, R2Y(1), 40,  PR_H}, false, "gpsd?", "No", NULL},
    {0, {10,  R2Y(2),  70, PR_H}, {110, R2Y(2), 50,  PR_H}, false, "WiFi?", "No", NULL},

    {0, {10,  R2Y(4),  70, PR_H}, {110, R2Y(4), 140, PR_H}, false, "Map:", "Terrain", "Countries"},
                                                                // max strlen NV_MAPSTYLE_LEN-1
    {0, {380, R2Y(4), 120, PR_H}, {520, R2Y(4), 120, PR_H}, false, "Log usage?", "Opt-Out", "Opt-In"},

    // page 2

    {1, {10,  R2Y(0), 80, PR_H},  {110, R2Y(0), 110, PR_H}, false, "Cluster?", "No", NULL},
    ...
    {1, {10,  R2Y(1), 80, PR_H},  {110, R2Y(1), 110, PR_H}, false, "NTP?", "Default set", NULL},

    {1, {10,  R2Y(2), 80, PR_H},  {110, R2Y(2), 110, PR_H}, false, "Units?", "Imperial", "Metric"},

    {1, {10,  R2Y(4), 80, PR_H},  {110, R2Y(4), 110, PR_H}, false, "KX3?", "No", NULL},        // kx3 on/off
    {1, {110, R2Y(4),  0, PR_H},  {110, R2Y(4), 110, PR_H}, false, NULL, "4800", "38400"},     // baud
    {1, {270, R2Y(4), 80, PR_H},  {370, R2Y(4), 110, PR_H}, false, "GPIO?", "Off", "Active"},
    {1, {530, R2Y(4), 80, PR_H},  {630, R2Y(4), 35,  PR_H}, false, "Flip?", "No", "Yes"},
};
...
{
    char buf[32];
    snprintf (buf, sizeof(buf), _FX("Page %d ..."), cur_page+1);
...
static void drawSPValue (StringPrompt *sp)
{
    ...
    if (sp->v_len == sl+1 || chop) {
        ...
        tft.printf (_FX("%.*s"), sl - 1, sp->v_str);
        uint16_t boxr_x = sp->v_box.x+sp->v_box.w;
        uint16_t lastch_x = tft.getCursorX();
        tft.fillRect (lastch_x, sp->v_box.y, boxr_x-lastch_x, sp->v_box.h, BG_C);
        tft.print(sp->v_str[sl-1]);
        // position cursor under last char but beware not to overflow v_box
        sp->v_cx = lastch_x + PR_W < boxr_x ? lastch_x : boxr_x-PR_W;
    } else {
...
static void initSetup()
{
    ...
    if (!NVReadString(NV_WIFI_PASSWD, pass) && !NVReadString(NV_WIFI_PASSWD_OLD, pass)) {
    ...
    char mapstyle[NV_MAPSTYLE_LEN];
    bool ms_ok = false;
    if (NVReadString(NV_MAPSTYLE, mapstyle)) {
        if (strcmp (mapstyle, bool_pr[MAPSTYLE_BPR].t_str) == 0) {
            bool_pr[MAPSTYLE_BPR].state = true;
            ms_ok = true;
        } else if (strcmp (mapstyle, bool_pr[MAPSTYLE_BPR].f_str) == 0) {
            bool_pr[MAPSTYLE_BPR].state = false;
            ms_ok = true;
        }
    }
    if (!ms_ok) {
        NVWriteString(NV_MAPSTYLE, bool_pr[MAPSTYLE_BPR].f_str);
        bool_pr[MAPSTYLE_BPR].state = false;
    }
        ...
        if (c == '\t') {
            ...
            if (bp == &bool_pr[GEOIP_BPR]) {
                ...
                if (bp->state) {
                    ...
                    if (bool_pr[CLMAP_BPR].state) {
                        drawBPPrompt (&bool_pr[CLMAP_BPR]);
                        drawBPState (&bool_pr[CLLABEL_BPR]);
                    } else {
...
void clockSetup()
{
    ...
    NVWriteUInt8 (NV_BR_MIN, bright_min);
    NVWriteUInt8 (NV_BR_MAX, bright_max);
    NVWriteString(NV_MAPSTYLE, getCoreMapStyle());
 ...
 * N.B. only sensible if mapDXSpots() is true
 */
...
/* return map style
 */
const char *getCoreMapStyle()
{
    return (bool_pr[MAPSTYLE_BPR].state ? bool_pr[MAPSTYLE_BPR].t_str : bool_pr[MAPSTYLE_BPR].f_str);
}
