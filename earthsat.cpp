...
static bool satLookup ()
{
	...
	if (strcasecmp (name, sat_name)) {
	    fatalSatError (_FX("Name does not match: '%s' '%s'"), sat_name, name);
	    goto out;
	}
...
static bool askSat()
{
    ...
    selectFontStyle (LIGHT_FONT, SMALL_FONT);
    tft.setCursor (tft.width()-435, title_y);
    tft.printf (_FX("<%d Mins"), SOON_MINS);

    // show what SAT_COLOR means
    tft.setTextColor (SAT_COLOR);
    tft.setCursor (tft.width()-315, title_y);
    tft.print (F("Now"));

    // show rise units
    tft.setTextColor (RA8875_WHITE);
    tft.setCursor (tft.width()-250, title_y);
...
bool checkSatNameTouch (const SCoord &s)
{
    if (dx_info_for_sat) {
        ...
        // check just the left third so TZ button is not surrounded
