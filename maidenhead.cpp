 ...
 */
bool maidenhead2ll (LatLong &ll, const char maid[5])
{
    if (maid[0] < 'A' || maid[0] > 'R' || maid[1] < 'A' || maid[1] > 'R'
                      || !isdigit(maid[2]) || !isdigit(maid[3]))
        return (false);

    ll.lng_d = (maid[0] - 'A') * 20 + (maid[2] - '0') * 2 - 180;
    ll.lng = deg2rad (ll.lng_d);

    ll.lat_d = (maid[1] - 'A') * 10 + (maid[3] - '0') - 90;
...
void setMaidenhead(NV_Name nv, LatLong &ll)
{
    ...
    uint32_t mnv = packMaidFromStr (maid[0]);
...
void toggleMaidenhead(NV_Name nv, LatLong &ll)
{
    ...
    mnv = packMaidFromStr (maid[choice]);
    NVWriteUInt32 (nv, mnv);
}

/* given a valid grid string, return as packed for NV
 */
uint32_t packMaidFromStr (char maidstr[4])
{
    uint32_t m;
    memcpy (&m, maidstr, 4);
    return (m);
...
/* given a valid grid packed for NV, return as a string
 */
void unpackMaidToStr (char maidstr[5], uint32_t grid)
{
    memcpy (maidstr, &grid, 4);
    maidstr[4] = '\0';
}
