...
SBox plot1_b = {235, 0, PLOTBOX_W, PLOTBOX_H};
SBox plot2_b = {405, 0, PLOTBOX_W, PLOTBOX_H};
SBox plot3_b = {575, 0, PLOTBOX_W, PLOTBOX_H};
...
// callsign info
typedef struct {
    const char *call;                   // callsign from clockSetup()
...
#define	GPATH_LINGER	20000		// time for great path to remain on screen, millis()
...
// #define _GFX_COORD_TEST
    ...
    // retrieve plot choices, check for inconsistencies and insure sane defaults
    ...
    if (!NVReadUInt8 (NV_PLOT_2, &plot2_ch) || (plot2_ch == PLOT2_DX && !useDXCluster())) {
        plot2_ch = plot1_ch != PLOT1_XRAY ? PLOT2_XRAY : PLOT2_KP;
    ...
    // Serial.printf (_FX("Plots: %d %d %d\n"), plot1_ch, plot2_ch, plot3_ch);
...
static void checkTouch()
{
	...
	drawCallsign (true);	// fg and bg
        ...
        if (!dx_info_for_sat)
            newDX (ll, NULL);
    } else if (inCircle(s, dx_marker_c)) {
        if (!dx_info_for_sat)
            newDX (dx_ll, NULL);
...
void newDE (LatLong &ll)
{
	...
	NVWriteFloat (NV_DE_LNG, de_ll.lng_d);
        ...
        // show DE weather and update band conditions and cluster if showing
        showDEWX();
        newBC();
        newVOACAPMap();
        sendDELLGrid();
...
static void drawVersion(bool force)
{
    ...
    bool new_avail = newVersionIsAvailable (NULL, 0);
    check_t = millis();
    ...
    snprintf (vermsg, sizeof(vermsg), _FX("Ver %s"), HC_VERSION);
    uint16_t vw = getTextWidth (vermsg);
...
bool overAnySymbol (const SCoord &s)
{
    return (inCircle(s, de_c) || inCircle(s, dx_c) || inCircle(s, deap_c)
                ...
                || overAnyDXSpots(s) || inBox(s,santa_b));
}
...
{
    // setup
    #define MSG_NROWS   10              // actually 1- because we preincrement row
    #define MSG_ROWH    35
...
static void shutdown(void)
{
    ...
    ShutDownCtrl sdc[] = {
        {"Never mind -- resume", {x0, y, w, h},    RA8875_GREEN, false},
        {"Restart HamClock",     {x0, y+=h, w, h}, RA8875_YELLOW, false},
        {"Exit HamClock",        {x0, y+=h, w, h}, RA8875_MAGENTA, false},
        {"Reboot Pi",            {x0, y+=h, w, h}, RA8875_RED, false},
        {"Shutdown Pi",          {x0, y+=h, w, h}, RGB565(255,125,0), false},
    };
    ...
    do {
        ...
        // display all
        for (unsigned i = 0; i < NARRAY(sdc); i++) {
    ...
    } while (!shutdown_rus (sdc[selection].box, sdc[selection].color));
    ...
    case 3:
        // reboot pi
        drawStringInBox ("Rebooting Pi...", sdc[3].box, true, RA8875_RED);
        tft.drawPR();            // forces immediate effect
        system ("sudo reboot");
        for(;;);
    case 4:
        // shutdown pi
        drawStringInBox ("Shutting down Pi...", sdc[4].box, true, RA8875_RED);
        tft.drawPR();            // forces immediate effect
        system ("sudo halt");
        for(;;);
    default:
