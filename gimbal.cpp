...
#define TITLE_Y         (plot3_b.y+PLOTBOX_H/5-2)       // title y coord, match VOCAP
...
#define AUTO_Y          (plot3_b.y+PLOTBOX_H-CHAR_H-15) // auto button y coord
    ...
    for (t = 0; t < ntry; t++) {
        ...
        if (arc_mc.parsePacket()) {
            ...
            if (arc_client.connect (rip, port)) {
                ...
                // start
                if (!doARCMessage (buf, buf_mem.getSize(), "start")) {
                    closeGimbal();
                    continue;
                }
                if (strncmp (buf, "OK ", 3)) {
                ...
                // get model
                if (!doARCMessage (buf, buf_mem.getSize(), "get model")) {
                    closeGimbal();
                    continue;
                }
                if (strncmp (buf, "OK ", 3)) {
                ...
                // get version
                float v;
                if (!getARCFloat (_FX("get version"), &v)) {
                    closeGimbal();
                    continue;
                }
...
static bool doARCMessage (char *resp, int maxresp_l, const char *msgfmt, ...)
{
        ...
        plotMessage (plot3_b, RA8875_RED, _FX("Connection lost"));
        closeGimbal();
        return (false);
}
...
static bool getARCFloat (const char *msg, float *valp)
{
        ...
        if (!doARCMessage (resp, resp_mem.getSize(), "%s", msg))
...
static bool setARCFloat (const char *msg, float val)
{
        ...
        StackMalloc resp_mem(150);
        char *resp = resp_mem.getMem();
        if (!doARCMessage (resp, resp_mem.getSize(), "%s %g", msg, val))
            return (false);                     // reason already logged
...
static bool getAz()
{
    ...
    if (doARCMessage (resp, resp_mem.getSize(), "get az")) {
...
static bool getEl()
{
    ...
    if (doARCMessage (resp, resp_mem.getSize(), "get el")) {
...
static void unStopGimbal()
{
    if (ARCIsConnected()) {
        ...
        // go
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), _FX("set stop 0"));
    }
}
...
void stopGimbalNow()
{
    if (ARCIsConnected()) {
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), _FX("set stop 1"));
    }
...
void updateGimbal ()
{
    ...
    if (send_now) {
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), _FX("set now %g %g %g"),
