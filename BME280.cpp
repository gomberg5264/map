...
#define	I2CADDR_1       0x76			// sensor I2C address, possibility 1
#define	I2CADDR_2       0x77			// sensor I2C address, possibility 2
...
static void connectSensors()
{
    ...
    // try both addresses, noting which
    bool found_1 = bme.begin(I2CADDR_1);
    if (found_1 || bme.begin(I2CADDR_2)) {
    ...
    if (bme280_connected)
        Serial.printf(_FX("found at 0x%02X\n"), found_1 ? I2CADDR_1 : I2CADDR_2);
    else
...
/* plot the current sensor choice if showing
 */
static void plotBME280 ()
{
    if (!bme280_connected)
	return;
    ...
    // build x and y
    StackMalloc x_mem(N_SENS*sizeof(float));
    StackMalloc y_mem(N_SENS*sizeof(float));
